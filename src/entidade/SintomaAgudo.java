...
public enum SintomaAgudo {
    ...
    DIFICULDADE_RESPIRATORIA_GRAVE(
            "Dificuldade respiratória grave (não consegue falar)",
            25
    ),
    ...
    SUSPEITA_AVC(
            "Suspeita de AVC (fala alterada, desvio facial, fraqueza súbita)",
            24
    ),
    ...
    TRAUMA_CRANIANO_GRAVE(
            "Trauma craniano com perda de consciência",
            21
    ),

    GESTANTE_COM_SANGRAMENTO(
            "Gestante com sangramento ou dor intensa",
            20
    ),

    QUEIMADURA_GRAVE(
            "Queimadura grave (>20% do corpo ou vias aéreas)",
            20
    ),
    ...
    FREQUENCIA_CARDIACA_CRITICA(
            "Frequência cardíaca acima de 150 bpm ou abaixo de 40 bpm",
            18
    ),
    ...
    TENTATIVA_SUICIDIO(
            "Tentativa de suicídio ou autolesão grave",
            15
    ),
    ...
    TRAUMA_CRANIANO_LEVE(
            "Trauma craniano sem perda de consciência",
            9
    ),
    ...
    SintomaAgudo(String descricao, int peso) {
